...
# Handle missing data in df within the app for fitting encoders
df['education'] = df['education'].fillna('None')
df['previous_year_rating'] = df['previous_year_rating'].fillna(df['previous_year_rating'].mode()[0])


# Refit encoders on the data with handled missing values
department_encoder.fit(df['department'])
region_encoder.fit(df['region'])
education_encoder.fit(df['education']) # Fit on df['education'] after filling NaNs
gender_encoder.fit(df['gender'])
recruitment_channel_encoder.fit(df['recruitment_channel'])
...
# Tab 3: Predict from CSV
    ...
    if uploaded_file is not None:
        ...
        csv_df_org = csv_df_org.dropna() # Drop rows with NaN for simplicity in this example
        # csv_df_org.columns

        csv_df = csv_df_org.copy()
        # Drop employee_id if it exists in the uploaded CSV
